...
class Index extends Component
{
    ...
    public function submit()
    {
        ...
        $this->reset(['name', 'email', 'mobile', 'selectedRoles', 'selectedPermissions']);

        session()->flash("message", "ادمین با موفقیت اضافه شد!" . "<br>" . "پسورد:" . $password);
    }
    ...
    {
        $new_data = Admin::query()->where('id', $adminId)->firstOrFail();

        if ($new_data) {
            $this->name = $new_data->name;
            $this->email = $new_data->email;
            $this->mobile = $new_data->mobile;
            $this->selectedRoles = $new_data->roles->pluck('id')->toArray();
            $this->selectedPermissions = $new_data->permissions->pluck('id')->toArray();
        }
    }
