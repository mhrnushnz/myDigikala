...
<main class="d-flex flex-column border p-4 rounded">
    ...
    <div class="text-center">
        ...
        @if(!@$submitCodeView)
            ...
            <form wire:submit.prevent="sendSms" autocomplete="on" class="d-flex flex-column align-items-center justify-content-center gap-3 mb-4">
        ...
        <div class="fs-8 text-black-50">
            <p>سلام !</p>
            <p class="my-3">لطفاکد ارسال شده به شماره موبایل {{ $userMobile }} را وارد کنید</p>
        </div>
            <form wire:submit.prevent="submitUser" autocomplete="on" class="d-flex flex-column align-items-center justify-content-center gap-3 mb-4">
                <input name="code" type="text" maxlength="4" wire:model="code" class="w-100 py-2 rounded-3 border border-danger" />
                @error("code")
                <div class="w-100 alert alert-danger">{{ $message }}</div>
                @enderror
